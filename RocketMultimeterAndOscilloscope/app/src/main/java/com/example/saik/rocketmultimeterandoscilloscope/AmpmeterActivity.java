...
public class AmpmeterActivity extends AppCompatActivity {


    int ampDataBT;
    TextView display;
    boolean isAmpmeterOn = true;
    Handler handler;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ampmeter);
        display = (TextView) findViewById(R.id.ampmeter);
        ampDataBT = 0;
        display.setText(new DecimalFormat("##.##").format(calculateAmperageValue()) + " A");
        final Button ampTog = (Button) findViewById(R.id.ampmeterButton);
        ampTog.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (isAmpmeterOn){
                    isAmpmeterOn = false;
                    ampTog.setText("Resume");
                }
                else{
                    isAmpmeterOn = true;
                    ampTog.setText("Pause");
                }
            }
        });
        ...
        graph.getGridLabelRenderer().setVerticalAxisTitle("Amperage");
    ...
    protected void onDestroy() {
        super.onDestroy();
        Log.d(TAG, "AmpmeterActivity: onDestroy: calling cleanup");
    ...
    private void addEntry() {
        if (lastX < 10.0){
            series.appendData(new DataPoint(lastX, calculateAmperageValue()), false, 10000); // we can store 10000 values at a time
        }
        else{
            series.appendData(new DataPoint(lastX, calculateAmperageValue()), true, 10000); // we can store 10000 values at a time
        }
        lastX+= 0.1;
    }

    private double calculateAmperageValue(){
        if (ampDataBT !=0){
                //return ((((ampDataBT*5.0)/1023.0) - 2.5)/0.185);
            return (((((ampDataBT - 11.596)*5.0)/1023.0) - 2.5)/0.185);
        }
        ...
        protected Void doInBackground(Void... devices)
        {
            ...
            while(!isCancelled())
            {
                if(btService != null)
                {
                    ...
                    {
                        ampDataBT = Integer.parseInt(completeMessage);
                        completeMessage = "";
                    }
                }
    ...
    void bindServiceAndRegisterReceiver() {
        Intent gattServiceIntent = new Intent(AmpmeterActivity.this, BTService.class);
